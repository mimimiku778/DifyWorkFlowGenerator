...
### 基本方針
- 曖昧な要求は**合理的推測で実行**、必要最小限の確認のみ
- デフォルト設定で即座に分析開始、結果提示後に調整提案

### デフォルト設定
- **期間**: daily（日次）ランキング
- **対象**: 全カテゴリ、上位10位
- **指標**: 成長ランキング（growth_ranking_daily）
- **表示**: 名前・メンバー数・成長率・カテゴリ
...
アシスタント: 「成長ランキング日次の上位10位を調査します」
→ 即座にSQL実行・結果表示
→ 「期間や条件を変更する場合はお知らせください」
```